...
const AppbarLang = () => {
  ...
  const handleCoutrySelected = (country, flag, lang) => {
  ...
  return (
    <div className="relative w-30 h-10">
      <div className="drop-selected cursor-pointer px-1 py-3 flex items-center gap-3 w-full h-full" onClick={handleDropListEnable}>
      ...
      <div className={`drop-lists absolute top-full w-full bg-gray-950 ${isDropListOpen ? '' : 'hidden'}`}>
              ...
              if (country?.languages && Object.keys(country?.languages)) {
                ...
                return (
                  <div
                    ...
                    className="drop-item flex items-center gap-3 py-1 px-0 hover:bg-gray-700 transition-all duration-200 cursor-pointer" onClick={() => handleCoutrySelected(
                      country?.name?.common,
                      country?.flags?.png,
                      langKey
                    )}
