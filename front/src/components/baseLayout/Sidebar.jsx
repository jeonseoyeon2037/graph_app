...
const Sidebar = () => {
  ...
  return (
    <div
      className={`sidebar-wrapper dark:bg-gray-950 py-5 px-4 shadow-[0_0.125rem_0.25rem_rgba(255,255,255,0.3)] lg:w-[20%] md:w-[30%] w-[50%] h-full flex flex-col rounded-sm z-[999] fixed lg:left-0 ${
          ...
          <div className="sidebar-menu w-full">
            <ul className="menu-lists flex flex-col gap-y-1.5">
              ...
              {MENU_LISTS.map((menu, idx) => (
                <li key={idx}>
                  <Link to={menu.path} 
                  className={`flex items-center py-0.5 px-5 font-medium gap-x-[14px] h-[44px] ${currentMenu === idx ? 'bg-blue-700 text-white rounded-sm' : ''}`}
                  onClick={() => setCurrentMenu(idx)}
                  >
                    <span className={`w-5 ${activate ? 'invert-[1] brightness-[100%]' : ''}`}>
